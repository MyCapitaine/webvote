...
(function($){
    $.fn.emailMatch= function(settings){
        ...
        return this.each(function(){
			...
			var w = elem.width();
			var Y = elem.offset().left;
            var mailWrap = document.createElement("div");
			$(mailWrap).attr({"class":settings.className});
            $(settings.wrapLayer).append(mailWrap);
			//showList($(settings.wrapLayer),w,h,X,Y);
            elem.focus(function(){
				//显示补全div
				X = elem.offset().top;
				w = elem.width();
            ...
            elem.keyup(function(e){
                ...
                switch(eKey){
                    ...
                    case 13: { // 回车 
                            enterVal(mailWrap,elem);
							hideList($(settings.wrapLayer),w,h,X,Y);
                            hideList($(mailWrap));
                    }break;
                    case 38:{ // 方向键 上
						showList($(settings.wrapLayer),w,h,X,Y);
            ...
            $(mailWrap).on("mouseover","li:not('.first')",function(){
                console.log('mouseover');
                $(this).addClass("current").siblings().removeClass("current"); 
            });
            $(mailWrap).on('mousedown',"li:not('.first')",function(){
                $(this).addClass("current").siblings().removeClass("current");
                enterVal(mailWrap,elem);
                hideList($(mailWrap));
            });
            $(mailWrap).on("mouseout",function(){
                console.log('mouseout');
                $(mailWrap).find("li:not('.first')").removeClass("current");
            });
        });
    };
    ...
    function enterVal(oWrap,elem){
        if($(oWrap).find("li.current").length!=0)
			elem.val($(oWrap).find("li.current").text()); // 获取高亮内容 到 输入框
    };
    
    function showList(oElem,w,h,t,l){ // 显示 邮箱框架 并定位.
        oElem.css({"display":"block","top":h + t,"left":l+39,"width":w});
    };
    
    function hideList(oElem){ // 显示 邮箱框架 
		oElem.find("li:not('.first')").removeClass("current");
